...
class HomeWindow(Tk):
    def __init__(self):
        ...
        list1 = Menu(self)
        list1.add_command(label="Defects", command=add_defect)
            

        list2 = Menu(self)
        list2.add_command(label = "Add User",command = add_user)
        ...
        def check():
            try:
                ...
                if not z:
                    ...
                    self.srt = Button(self, text='Search', width=15, font=('arial', 10),command = search_defect).place(x=700, y=206)
